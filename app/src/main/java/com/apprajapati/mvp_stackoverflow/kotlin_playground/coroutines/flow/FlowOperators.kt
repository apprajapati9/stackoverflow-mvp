...
suspend fun collectStateShowcase() {
     ...
     */
    countDownFlow().collectLatest { number ->
...
fun flatMapAndMapShowcase() {
    ...
    val flattened = listOf(list1, list2).flatMap { value ->
        value.toList()
    }
...
suspend fun main() {
    ...
    val count = countDownFlow().count { num ->
    ...
    val fold = countDownFlow().fold(100) { accumulator, value ->
        accumulator + value
    }
    println("fold -> $fold")
