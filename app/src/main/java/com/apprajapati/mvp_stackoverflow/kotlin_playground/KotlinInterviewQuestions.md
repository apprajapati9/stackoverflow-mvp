...
reference has a null value.
    ...
    ? - to notify compiler that this could be null at run-time thus compiler notifies to check for nullability. 
    ?. - safe call operator
    as? safe cast, diff than as which can throw an exception but as? tries to cast a value to the specified type and returns null if value is not of that type.

For more [read this link](https://kotlinlang.org/docs/null-safety.html#the-operator)

### Q8. What are Safe Casts in Kotlin?

**Short answer:** Kotlin Safe cast `as?` operator provides a safe cast operator for safely cast to a
type.
...
the syntax is `var/val <property-name> : <Type> by <expression>`. The expression after `by` is a
delegate,
because get and set that correspond to the property will be delegated to its getValue and setValue
methods.
Property delegates don't have to implement an interface, but they have to provide a getValue and
setValue for vars.

The most obvious and used delegated properties is when you use viewmodels like this

```kotlin
private val viewMode: MainViewModel by viewModels() 
```

You can check `FragmentViewModelLazy.kt` and `ViewModelLazy.kt` files to check the implementation
of `viewmodels()` property delegation.

It is used for

1. lazy properties. The value is computed only on its first access.
2. observable : listeners are notified about changes to this property
3. storing properties in a map instead of separate field for each property.

You can also use this for saving preferences. Whenever property changes,
it will be notified using observable and you can save or update prefs.

In this code, by viewModels is the example of property delegation.
...
Advantage of inline function: Function call overhead doesn't occur. Less overhead and faster program
execution.
...
So if we have data class with identical fields/properties then `==` would return true but if you
want to check if they are ref of same data class then use `===`.
...
**Short answer:** Using the object keyword, Singleton Pattern
